...
import { Mail, Phone, UserPlus, LockKeyhole } from "lucide-react";
...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showMessage(response.message || "Registro exitoso.", "success");

      setTimeout(() => {
        navigate(
          `/verify-email?correo=${encodeURIComponent(payload.correo)}`
        );
      }, 900);
    } catch (error) {
      showMessage(error.userMessage || "No se pudo registrar la cuenta.", "error");
    }
  };
      ...
      <div className="w-full max-w-2xl">
        <div className="bg-white/95 border border-slate-200 rounded-[2rem] p-8 shadow-xl">
          ...
          <p className="text-slate-500 mt-2">
            Crea tu cuenta para postular a vacantes de tecnología.
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-7 grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
            ...
            <div className="md:col-span-2">
              ...
              >
                <UserPlus size={18} />
                {loadingAuth ? "Registrando..." : "Crear cuenta"}
              </button>
            </div>
          </form>
